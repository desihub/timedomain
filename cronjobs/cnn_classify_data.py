...
parser = argparse.ArgumentParser(description='Run CNN classifier on selected tile and day')

parser.add_argument('--tilenum', nargs='+', type=str,default='80611',
                    help='Tile Numbers to be pocessed')
parser.add_argument('--obsdate', type=str,default='20201221',
                    help='Night to be pocessed')

args = parser.parse_args()

bgs_tiles = args.tilenum
...
for bgs_tile in bgs_tiles:
    ...
    prefix_in='/'.join([redux, bgs_tile+"/"+obsdate])
    if not os.path.isdir(prefix_in):
        ...
        #Will need to save results in a file - maybe depends on what input Skyportal wants?
        #Maybe save spectra of all transients, one folder per transient type.

            #Double check arrays aren't empty
            if allzbest is None:
                ...
                if ntr>0:
                    if tr_z is None:
                        ...
                        tr_tile=np.vstack((tr_tile,np.full(ntr,bgs_tile)))
                ...
                fig.tight_layout()
                fig.savefig(plot_path+'transients_'+bgs_tile+'_'+obsdate+'.png',dpi=200)
                print('Figure saved in ', plot_path)
