...
query="select distinct obsdate,tileid from exposures
where (tileid,obsdate) not in (select tileid,obsdate from desidiff_color_coadd_exposures);"
...
#             python ${run_path_diff}diff-db.py TileDate_TargetPairs_Iterator CVLogic daily spectra --obsdates_tilenumbers ${subarr[@]}
            python ${run_path_diff}diff-db.py TileDate_SpectraPairs_Iterator ColorLogic recent coadd --obsdates_tilenumbers ${subarr[@]}
            if [ $? -eq 0 ]
            then
...
#                     query="INSERT OR IGNORE INTO desidiff_cv_spectra_exposures(obsdate, tileid) VALUES(${arrt[0]},${arrt[1]});"
                    query="INSERT OR IGNORE INTO desidiff_color_coadd_exposures(obsdate, tileid) VALUES(${arrt[0]},${arrt[1]});"
                    echo $query
