...
while current_date <= end_date:
    ...
    prev_night.append(int((current_date).strftime('%Y%m%d')))
...
for night in prev_night:
    print(night)
    all_plots_pdf = PdfPages("/project/projectdirs/desi/users/clepart/pdfs/All_plots_"+str(night)+".pdf")
    count_night = 0
    count_file = 0
    ...
    for current_filename in glob(f"/global/cfs/projectdirs/desi/spectro/redux/daily/tiles/cumulative/*/{night}/spectra-*.fits*"):
        ...
        for ref_filename in glob(ref_path):
            ...
            if ref_filename.split('/')[-6] in ['iron'] or (ref_filename.split('/')[-6] == 'daily' and int(ref_filename.split('/')[-2]) < night):
                # read the spectra from the file
                prev_spectra = read_spectra(ref_filename)
                count_file += 1  # increment the file counter
            else:
            ...
            for t in relevant_targets:
                ...
                if any(logic):
                    ...
                    for key in (current_spectra.flux).keys():
                        ...
                        plt.plot(ref_spectra.wave[key][w],ref_spectra.flux[key][0][w],color='green',alpha=0.5, label = 'Reference Spectrum "' + ref_filename.split('/')[-6]+'"')
        ...
        print('Tile/petal combinations per filename: {}'.format(count_file))
