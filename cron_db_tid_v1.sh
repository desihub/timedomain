...
nloop=$(((Nobsdates_tileids+nper-1)/nper))
    ...
    for ((i=0;i<$nloop;i++)); 
        ...
        for t in ${subarr[@]}; do
                    ...
                    compare="INSERT INTO processed_daily SELECT desi.TARGETID,desi.TILEID,desi.YYYYMMDD, desi.PETAL FROM desi_db.fibermap_daily desi WHERE desi.TILEID=${arrt[1]} AND ;"
                    echo $compare
